package cn.huahongbin.sso.client1.filter;
...
public class SSOFilter implements Filter {
    ...
    public void doFilter(ServletRequest req, ServletResponse res, FilterChain chain)
        ...
        String logoutURL = URLEncoder.encode("http://localhost:8081/client1/user/logout", "UTF-8");
        
        //判断是否已经登录
        if (null == username) {
            //判断是否在处理token验证逻辑(包含token参数)
            if (null != token && !"".equals(token)) {
                //httpclient发送注销请求是没有会话id的,所以通过只token来验证
